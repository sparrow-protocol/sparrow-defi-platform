...
} from "@/app/types/api" // Added JupiterPriceResponse, JupiterRecurringOrderResponse
...
/**
 * Fetches recurring orders for a given user from Jupiter's Recurring API.
 * @param userPublicKey The public key of the user.
 * @param orderStatus Filter by order status (active, history, all).
 * @param recurringType Filter by recurring type (time, price, all).
 * @param page Page number for pagination.
 * @returns An array of JupiterRecurringOrderResponse or null.
 */
export async function getJupiterRecurringOrders(
  ...
  try {
    ...
    if (!response.ok) {
      const errorText = await response.text()
      console.error(`Error fetching Jupiter recurring orders: ${response.statusText}. Details: ${errorText}`)
      throw new Error(`Error fetching Jupiter recurring orders: ${response.statusText}`)
    }
    const data = await response.json()
    return data as JupiterRecurringOrderResponse[]
  } catch (error) {
    console.error("Failed to get Jupiter recurring orders:", error)
    return null
  }
}
