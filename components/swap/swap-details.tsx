...
export function SwapDetails({ quote, sellingToken, buyingToken }: SwapDetailsProps) {
  ...
  return (
    <div className="space-y-4">
      {priceImpact > 0.01 && ( // Show warning if price impact is significant
      ...
      {quote.platformFee && (
        <div className="flex justify-between items-center text-sm text-black/70 dark:text-light-gray p-2 rounded-md bg-medium-gray dark:bg-input-bg-dark">
          ...
          <span className="font-semibold text-black dark:text-white">
            {/* Assuming platformFee.mint is the same as sellingToken.mint for ExactIn */}
          ...
          <AccordionItem value="item-1" className="border-b border-light-gray dark:border-dark-gray">
            ...
            <AccordionContent className="pt-2 pb-4 text-black/70 dark:text-light-gray">
              <div className="grid gap-3 text-xs">
                {quote.routePlan.map((route: any, index: number) => (
                  <div key={index} className="rounded-md bg-medium-gray p-3 dark:bg-input-bg-dark">
                    ...
                    <p>
                      ...
                      {formatNumber(
                        Number(route.amount) / (sellingToken?.decimals ? Math.pow(10, sellingToken.decimals) : 1),
                        6,
                      )}{" "}
                      {sellingToken?.symbol}
                    </p>
                    <p>
                      ...
                      {formatNumber(
                        Number(route.swapInfo.outAmount) /
                          (buyingToken?.decimals ? Math.pow(10, buyingToken.decimals) : 1),
                        6,
                      )}{" "}
                      {buyingToken?.symbol}
                    </p>
